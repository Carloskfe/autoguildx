...
export class FilterListingsDto {
  ...
  @ApiProperty({ required: false, enum: ['part', 'service'] }) @IsOptional() @IsIn(['part', 'service']) type?: string;
  ...
  @ApiProperty({ required: false, default: 1 }) @IsOptional() @Type(() => Number) @IsInt() @Min(1) page?: number;
  @ApiProperty({ required: false, default: 20 }) @IsOptional() @Type(() => Number) @IsInt() @Min(1) limit?: number;
}
