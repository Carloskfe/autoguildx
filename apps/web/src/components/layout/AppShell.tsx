  ...
  return (
    <div className="min-h-screen flex flex-col">
      ...
      <header className="sticky top-0 z-40 bg-surface/95 backdrop-blur border-b border-surface-border px-4 py-3 flex items-center justify-between">
        <Link href="/feed" className="text-brand-500 font-black text-lg tracking-tight">AutoGuildX</Link>
        <Link href="/marketplace/new" className="btn-primary text-sm flex items-center gap-1.5 px-3 py-1.5">
        ...
        <nav className="hidden md:flex flex-col gap-1 w-56 shrink-0 p-4 border-r border-surface-border">
          {NAV.map(({ href, label, icon: Icon }) => (
            <Link key={href} href={href} className={clsx(
              ...
              pathname.startsWith(href) ? 'bg-surface-border text-white' : 'text-gray-400 hover:text-white hover:bg-surface-card',
            )}>
