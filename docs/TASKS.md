...
### Backend
- [x] `CoursesService.enroll()` payment guard — `ForbiddenException('Payment required')` unless `price === 0`, caller is instructor, or call comes from webhook path
- [x] `POST /courses/:id/checkout` — create Stripe Checkout Session (one-time payment); metadata: `{ courseId, userId }`
- [x] Stripe webhook handler for `checkout.session.completed` — verify metadata, call `CoursesService.enroll()`; reuse or extend existing Stripe instance in `SubscriptionsModule`
- [x] Wire `ReviewsModule` to accept `targetType: 'course'` — add `'course'` to `CreateReviewDto` `IsIn` validator
- [x] `previewVideoUrl` field on `CourseEntity` — already present from Sprint 24b
- [x] `sort` query param on `GET /courses` — `popular` · `rating` · `newest`
- [x] Completion email — sent via `EmailModule` when `checkAndIssueCertificate` issues a certificate
- [x] Unit tests — reviews spec updated with `targetType: 'course'` case (327 passing)

### Frontend
- [x] `/courses/[id]` — paid courses use `checkout.mutate()` → Stripe redirect; free courses use direct `enroll.mutate()`
- [x] `?payment=success` param handled on return from Stripe — green enrollment banner, cache invalidation, URL cleaned
- [x] Star rating display on course cards (`/courses` page)
- [x] Star rating + review count in course detail hero
- [x] `ReviewSection` on course detail page (`targetType="course"`)
- [x] Preview video player in sticky sidebar
- [x] `previewVideoUrl` field in course create/edit forms
- [x] Sort dropdown on `/courses` catalog — Newest · Most Popular · Top Rated
- [x] Progress bar on enrolled course cards

### Ops
- [ ] Set `STRIPE_SECRET_KEY` + `STRIPE_WEBHOOK_SECRET` in `.env.production` (if not already set)
- [ ] Register `POST /courses/:id/webhook` endpoint in Stripe dashboard
